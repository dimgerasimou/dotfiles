...
matrixinit(const uint32_t rows, const uint32_t cols)
{
	...
	mat->data = mmalloc(rows * sizeof(double*));
	for (uint i = 0; i < rows; i++) {
		mat->data[i] = mmalloc(cols * sizeof(double));
	}
...
matrixprint(Matrix *mat, const char *name)
{
	...
	for (uint32_t i = 0; i < mat->rows; i++) {
		for (uint32_t j = 0; j < mat->cols; j++) {
			printf("%10lf ", mat->data[i * mat->rows + i]);
		}
		printf("\n");
	}
	printf("\n");
                ...
                const uint32_t K, const char *filepath)
{
	...
	idx = matrixinit(N, K, NULL);
	dst = matrixinit(N, K, NULL);

	for (uint32_t i = 0; i < N; i++) {
		for (uint32_t j = 0; j < K; j++) {
			idx->data[j * N + i] = neighbors[i][j].idx + 1;
			dst->data[j * N + i] = neighbors[i][j].dst;
		}
	}
...
neighborsinit(const uint32_t N, const uint32_t K)
{
	...
	ret = mmalloc(N * sizeof(Neighbor*));

	for (uint32_t i = 0; i < N; i++)
		ret[i] = mmalloc(K * sizeof(Neighbor));
