...
const char *ICON_LIST[]     = { "battery", "intel", "deepin-graphics-driver-manager", "nvidia" };
...
exec_block_button(const unsigned int capacity, const char *status)
{
	...
	case 1:
	{
		char *capacity_str = uitoa(capacity);

		if (!capacity_str)
			break;

		notification(capacity_str, status);
...
get_capacity(void)
{
	...
	strapp(&path, battery_device_path);
	strapp(&path, "/capacity");


	path = mmalloc(strlen(battery_device_path) + 9);

	if (!(fp = fopen(CAPACITY_PATH, "r")))
		logwrite("fopen() failed for file", CAPACITY_PATH, LOG_FATAL, "dwmblocks-battery");
...
get_status(void)
{
	...
	if (!(fp = fopen(STATUS_PATH, "r")))
		logwrite("fopen() failed for file", STATUS_PATH, LOG_FATAL, "dwmblocks-battery");
