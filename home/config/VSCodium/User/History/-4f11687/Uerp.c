...
static void quick_sort(Neighbor *neighbors, int left, int right);
...
Matrix_t**
knnsearch(Matrix_t *C, Matrix_t *Q, uint32_t *K, uint32_t blocksize, uint16_t quicksort)
{
	uint16_t nthreads;
	...
	*K = C->rows < *K ? C->rows : *K;
	...
	for (uint32_t i = 0; i < Q->rows; i++) {
		if (quicksort)
			quick_sort(neighbors, i * *K, (i * *K) + *K - 1);
...
qselect_partition(Neighbor *neighbors, const int left, const int right)
{
	...
	// Swap elements based on primary (dst) and secondary (idx) comparisons
	...
	// Move the pivot to the right-1 position if there are more than two elements
	if (right - 1 > left)
		qselect_swap(&neighbors[mid], &neighbors[right - 1]);
	double pivot = neighbors[right - 1].dst;
	int pivotIdx = neighbors[right - 1].idx;  // Use pivotIdx for secondary comparison

	int i = left, j = right - 1;

	while (1) {
		// Primary and secondary comparison for the left side
		while (neighbors[++i].dst < pivot || 
		       (neighbors[i].dst == pivot && neighbors[i].idx < pivotIdx));
		// Primary and secondary comparison for the right side
		while (neighbors[--j].dst > pivot || 
		       (neighbors[j].dst == pivot && neighbors[j].idx > pivotIdx));
		if (i >= j) break;
		qselect_swap(&neighbors[i], &neighbors[j]);
	}
	qselect_swap(&neighbors[i], &neighbors[right - 1]);  // Move pivot to correct place
	return i;
}
...
static void
quick_sort(Neighbor *neighbors, const int left, const int right)
{
	if (left < right) {

		int pivotIndex = qselect_partition(neighbors, left, right);

		quick_sort(neighbors, left, pivotIndex - 1);
		quick_sort(neighbors, pivotIndex + 1, right);
	}
}
