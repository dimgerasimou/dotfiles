...
static void quicksort(Neighbor *arr, const uint32_t low, const uint32_t high);
static void qselect(Neighbor *arr, const uint32_t low, const uint32_t high, const uint32_t k);
...
/*
 * compares both the dst and if equal the index.
 * Thats how matlab does it. If not, if the kth and (k+1)th or up to (k+x)th element
 * element are equal, it is possible that the neighbors be different.
 * Moreover in the quicksort() it prevents swapped elements,
 * eg if dst1 = dst2, then it was probable that idx1 != idx2.
 */
static uint32_t
partition(Neighbor *arr, const uint32_t low, const uint32_t high)
{
	double pivot = arr[high].dst;
	int32_t pivot_idx = arr[high].idx;
	int i = low - 1;

	for (uint32_t j = low; j < high; j++) {
		if (arr[j].dst < pivot || (arr[j].dst == pivot && arr[j].idx < pivot_idx)) {
			i++;
			swap(&arr[i], &arr[j]);
		}
	}
...
static void
quicksort(Neighbor *arr, const uint32_t low, const uint32_t high)
{
	if (low < high) {
		uint32_t pivot_index = partition(arr, low, high);
...
static void
qselect(Neighbor *arr, const uint32_t low, const uint32_t high, const uint32_t k)
{
	if (low <= high) {
		uint32_t pivot_index = partition(arr, low, high);

		if (pivot_index > k)
			qselect(arr, low, pivot_index - 1, k);
		else
			qselect(arr, pivot_index + 1, high, k);
	}
}
