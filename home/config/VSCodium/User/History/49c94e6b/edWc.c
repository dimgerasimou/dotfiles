...
void
notify(const char *summary, const char *body, const char *icon, NotifyUrgency urgency, const int dormatsummary)
{
	char               *sum;
...
NotifyNotification*
newnotify(const char *summary, const char *body, const char *icon, NotifyUrgency urgency, const int dormatsummary)
{
	char               *sum;
	NotifyNotification *notification;

	if (dormatsummary) {
		sum = dormatsummary(summary, body);
	} else {
		sum = malloc((strlen(summary) + 1) * sizeof(char));
		strcpy(sum, summary);
	}
...
void
updatenotify(NotifyNotification *notification, const char *summary, const char *body, const char *icon, NotifyUrgency urgency, const int timeout, const int dormatsummary)
{
	char *sum;
