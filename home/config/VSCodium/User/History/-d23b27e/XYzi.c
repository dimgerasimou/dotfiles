...
{
	double temp_dst = ((double*) neighbors->dst)[i];
	int32_t temp_idx = ((int32_t*) neighbors->idx)[i];

	((double*) neighbors->dst)[i] = ((double*) neighbors->dst)[j];
	((int32_t*) neighbors->idx)[i] = ((int32_t*) neighbors->idx)[j];

	((double*) neighbors->dst)[j] = temp_dst;
	((int32_t*) neighbors->idx)[j] = temp_idx;
}

static void
qsort_swap_single(Neighbors *neighbors, uint32_t i, uint32_t j)
{
	float temp_dst = ((float*) neighbors->dst)[i];
	int32_t temp_idx = ((int32_t*) neighbors->idx)[i];

	((float*) neighbors->dst)[i] = ((float*) neighbors->dst)[j];
	((int32_t*) neighbors->idx)[i] = ((int32_t*) neighbors->idx)[j];

	((float*) neighbors->dst)[j] = temp_dst;
	((int32_t*) neighbors->idx)[j] = temp_idx;
}
...
static void qselect_double(Neighbors *neighbors, int left, int right, int K)
{
	if (left < right) {
		int pivotIndex = qsort_partition_double(neighbors, left, right);
		...
		if (pivotIndex == K)
			...
			qselect_double(neighbors, pivotIndex + 1, right, K);
		else
			qselect_double(neighbors, left, pivotIndex - 1, K);
	}
}

static void qselect_single(Neighbors *neighbors, int left, int right, int K)
{
	if (left < right) {
		int pivotIndex = qsort_partition_single(neighbors, left, right);
		...
		if (pivotIndex == K)
			...
			qselect_single(neighbors, pivotIndex + 1, right, K);
		else
			qselect_single(neighbors, left, pivotIndex - 1, K);
	}
}

void
qselect(Neighbors *neighbors, int left, int right, int K)
{
	if (neighbors->type == SINGLE)
		qselect_single(neighbors, left, right ,K);
	else if (neighbors->type == DOUBLE)
		qselect_double(neighbors, left, right ,K);
}
