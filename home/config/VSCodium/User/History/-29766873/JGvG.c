...
execbutton(unsigned int  *aur, unsigned int *pm)
{
	...
	case 1:
	{
		...
		notification = newnotify("Packages", "Getting packages upgrade info..", "tux", NOTIFY_URGENCY_NORMAL, 1);
		...
		*pm  = getupdates(pm_updates_cmd);
		...
		updatenotify(notification, "Packages", body, "tux", NOTIFY_URGENCY_NORMAL, 0, 1);
		free(body);
