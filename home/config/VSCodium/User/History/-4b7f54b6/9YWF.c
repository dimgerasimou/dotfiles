...
hdf5getmat(const char *filepath, const char *varname)
{
	...
	case H5T_INTEGER:
		if (H5Tget_size(datatype_id) == 8) {
			...
			status = H5Dread(dataset_id, H5T_NATIVE_SHORT, H5S_ALL, H5S_ALL, H5P_DEFAULT, (int32_t *) ret->data);
			if (status < 0)
				die("Error reading hdf5 data");
		}
		break;
...
matrixsave(IOHeader header, Matrix_t *idx, Matrix_t *dst)
{
	...
	case DOUBLE:
		...
		for (uint32_t i = 0; i < idx->rows; i++) {
			for (uint32_t j = 0; j < idx->cols; j++) {
				idx_temp[j * idx->rows + i] = ((int32_t*) idx->data)[i * idx->cols + j];
	...
	} else if (dst->type == SINGLE) {

	} else {

	}

	var = Mat_VarCreate("idx", MAT_C_INT32, MAT_T_INT32, 2, dims, idx->data, MAT_F_DONT_COPY_DATA);
	...
	if (dst->type == DOUBLE)
		...
		var = Mat_VarCreate("dst", MAT_C_SINGLE, MAT_T_SINGLE, 2, dims, dst->data, MAT_F_DONT_COPY_DATA);
	else
		var = 0;
