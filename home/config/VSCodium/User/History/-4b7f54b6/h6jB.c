...
hdf5getmat(const char *filepath, const char *varname)
{
	...
	if ((file_id = H5Fopen(filepath, H5F_ACC_RDONLY, H5P_DEFAULT)) < 0)
		die("Failed to open hdf5 file", filepath);
...
matiosave(const char *path, Matrix_t *idx, Matrix_t *dst)
{
	...
	if (!(fp = Mat_CreateVer(path, NULL, MAT_FT_MAT73)))
		die("Failed to create the mat file", path);
	...
	default:
		var = 0;
		die("Invalid 'distances' matrix type", NULL);
	}

	if (!var)
		die("Failed to create the variable 'distances'", NULL);

	if (Mat_VarWrite(fp, var, MAT_COMPRESSION_NONE) != 0) {
		Mat_VarFree(var);
		Mat_Close(fp);
		die("Failed to write the 'distances' variable to the file", path);
	}
	...
	if (!var)
		die("Failed to create the variable 'neighbors'", NULL);


	if (Mat_VarWrite(fp, var, MAT_COMPRESSION_NONE) != 0) {
		Mat_VarFree(var);
		Mat_Close(fp);
		die("Failed to write the 'neighbors' variable to the file", path);
	}
...
hdf5save(const char *path, const Matrix_t *idx, const Matrix_t *dst)
{
	...
	if ((file_id = H5Fcreate(path, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT)) < 0)
		die("Failed to create HDF5 file", path);
	...
	default:
		H5Fclose(file_id);
		die("Unsupported matrix data type for variable 'distances'", NULL);
	}

	dataspace_id = H5Screate_simple(2, dims, NULL);
	if (dataspace_id < 0) {
		H5Fclose(file_id);
		die("Faild to create HDF5 dataspace", path);
	}
	...
	if (dataset_id < 0) {
		H5Sclose(dataspace_id);
		H5Fclose(file_id);
		die("Failed to create HDF5 dataset", path);
	}
	...
	if (status < 0) {
		...
		H5Sclose(dataspace_id);
		H5Fclose(file_id);
		die("Failed to write data to HDF5 file", path);
	}
