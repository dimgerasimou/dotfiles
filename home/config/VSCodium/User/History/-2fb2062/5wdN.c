 ...
 * based on the: total memory=(2 * g_blocksize * cols + 2 * g_blocksize^2 + 2 * g_blocksize) * sizeof(double)
*/
void
dynamic_blocksize(size_t element_size, uint32_t cols)
{
	...
	if (cache_size == -1) cache_size = 256 * 1024;  // default to 256KB if unknown

	blocksize = (- (double) (cols + 1) + sqrt((double) (cols + 1) * (cols + 1) + (double) (2 * cache_size) / element_size)) / 2.0;

	// Round to nearest power of 2 for block alignment
	uint16_t ret = 1;
	while (ret < (int) blocksize) ret *= 2;
