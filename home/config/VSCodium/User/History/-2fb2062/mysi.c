...
uint32_t dynamic_blocksize(size_t element_size, uint32_t cols) {
	// Fetch the L2 or L3 cache size (assuming it's available on your system)
	long cache_size = sysconf(_SC_LEVEL2_CACHE_SIZE);
	if (cache_size == -1)
		return 32;

    // Calculate rows of data that fit within half the cache size (leaving room for other data)
    uint32_t blocksize = (uint32_t)(cache_size / (2 * element_size * cols));
    return blocksize > 0 ? blocksize : 32; // Use a fallback minimum block size
}
