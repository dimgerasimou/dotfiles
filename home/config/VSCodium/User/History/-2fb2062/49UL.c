...
getnorm(Matrix *mat, double *norms)
{
	for (uint32_t i = 0; i < mat->rows; i++) {
		...
		for (k = 0; k + 3 < mat->cols; k += 4) {
			sum += mat->data[k * mat->rows + i] * mat->data[k * mat->rows + i]
			    + mat->data[(k + 1) * mat->rows + i] * mat->data[(k + 1) * mat->rows + i]
			    + mat->data[(k + 2) * mat->rows + i] * mat->data[(k + 2) * mat->rows + i]
			    + mat->data[(k + 3) * mat->rows + i] * mat->data[(k + 3) * mat->rows + i];
		}
