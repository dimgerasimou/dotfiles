...
{
	double *row = (double*) malloc(mat->cols * sizeof(double));

	for (uint32_t i = 0; i < mat->rows; i++) {
		for (uint32_t j = 0; j < mat->cols; j++)
			row[j] = mat->data[j * mat->rows + i];

		norms[i] = cblas_ddot(mat->cols, row, 1, row, 1);
	}
...
getdistance(const double *C, const double *Q, const Matrix *D, double *D_M, double *Cnorm, double *Qnorm, const uint32_t cols)
{
	...
	cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasTrans, D->cols, D->rows, cols, -2.0,
	            C, D->cols, Q, D->rows, 0.0, D_M, D->cols);

	for (uint32_t i = 0; i < D->rows; i++) {
		for (uint32_t j = 0; j < D->cols; j++) {
			D->data[j * D->rows + i] = Cnorm[i] + D_M[i * D->cols + j] + Qnorm[j];
		}
	}
}
...
knnsearch(Matrix *C, Matrix *Q, uint32_t *K)
{
	...
	*K = C->rows < *K ? C->rows : *K;
	...
	Ctemp = matrixinit(g_blocksize,  C->cols);
	Qtemp = matrixinit(g_blocksize,  Q->cols);
	...
	for (uint32_t i = 0; i < Q->rows; i += g_blocksize) {
		N = (i + g_blocksize > Q->rows) ? (Q->rows - i) : g_blocksize;

		for (uint32_t j = 0; j < C->rows; j += g_blocksize) {
			M = (j + g_blocksize > C->rows) ? (C->rows - j) : g_blocksize;
