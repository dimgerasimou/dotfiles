#include <omp.h>
#include <stdint.h>
#include <stdio.h>
#include <string.h>
#include <time.h>
#include <sys/time.h>

#include "knnsearch.h"

static int
isuint(char *s)
{
	if (!s && s[0] == '\0')
		return 1;

	for (char *ptr = s; *ptr != '\0'; ptr++) {
		if (*ptr > '9' || *ptr < '0')
			return 0;
	}

	if (s[0] == '0' && s[1] == '\0')
		return 0;

	return 1;
}

static const char*
getextention(const char *path)
{
	const char *slash = strrchr(path, '/');
	const char *filename = (slash) ? slash + 1 : path;

	const char *dot = strrchr(filename, '.');
	if (!dot || dot == filename) {
		return "";
	}
	return dot + 1;
}

static int
parsefile(const char *path, IOHeader *header)
{
	const char *extention = getextention(path);

	if (!strcmp(extention, "mat"))
		header->type = MAT;
	else if (!strcmp(extention, "h5") || !strcmp(extention, "hdf5"))
		header->type = HDF5;
	else
		return 1;

	header->path = strdup(path);
	return 0;
}

static void
parseargs(int argc, char *argv[], IOHeader *input, IOHeader *output, uint32_t *K, uint32_t *blocksize, uint16_t *quicksort)
{
	for (int i = 1; i < argc; i++) {
		if (!strcmp(argv[i], "-i")) {
			if (argc < i + 1)
				die("Invalid number of arguments", argv[i]);
			if (parsefile(argv[++i], input))
				die("Invalid filetype", argv[i]);
		} else if (!strcmp(argv[i], "-o")) {
			if (argc < i + 1)
				die("Invalid number of arguments", argv[i]);
			if (parsefile(argv[++i], output))
				die("Invalid filetype", argv[i]);
		} else if (!strcmp(argv[i], "-b")) {
			if (argc < i + 1)
				die("Invalid number of arguments", argv[i]);
			if (isuint(argv[++i]))
				*blocksize = atoi(argv[i]);
		} else if (!strcmp(argv[i], "-k")) {
			if (argc < i + 1)
				die("Invalid number of arguments", argv[i]);
			if (isuint(argv[++i]))
				*K = atoi(argv[i]);
		} else if (!strcmp(argv[i], "-t")) {
			if (argc < i + 1)
				die("Invalid number of arguments", argv[i]);
			if (isuint(argv[++i]))
				omp_set_num_threads(atoi(argv[i]));
		} else if (!strcmp(argv[i], "-qsort")) {
			*quicksort = 1;
		} else {
			die("Invalid argument", argv[i]);
		}
	}

	if (!input->type || !output->type)
		die("Specify the input - output files", NULL);

	if (!*K)
		die("Enter the value of K", NULL);
}

int
main(int argc, char *argv[])
{
	struct timeval t0, t1;
	uint32_t K = 0, blocksize = 0;
	Matrix_t *C = NULL, *Q = NULL;
	Matrix_t **neighbors = NULL;
	IOHeader input = {0, NULL}, output = {0, NULL};
	uint16_t quicksort = 0;

	srand(time(NULL));

	parseargs(argc, argv, &input, &output, &K, &blocksize, &quicksort);

	if (input.type == HDF5) {
		C = matrixget(input, "train");
		Q = matrixget(input, "test");
	} else if(input.type == MAT) {
		C = matrixget(input, "C");
		Q = matrixget(input, "Q");
	} else {
		die("Failed to initialize input matrices", NULL);
	}

	gettimeofday(&t0, NULL);

	// benchmarked function here
	neighbors = knnsearch(C, Q, &K, blocksize, quicksort);

	gettimeofday(&t1, NULL);
	
	printf("%.8g\n", t1.tv_sec - t0.tv_sec + 1E-6 * (t1.tv_usec - t0.tv_usec));

	matrixtosingle(neighbors[1]);

	matrixsave(output, neighbors[0], neighbors[1]);

	matrixfree(&neighbors[0]);
	matrixfree(&neighbors[1]);
	free(neighbors);

	matrixfree(&C);
	matrixfree(&Q);

	return 0;
}
