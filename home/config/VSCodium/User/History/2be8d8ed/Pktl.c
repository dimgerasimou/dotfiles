...
tomlgetbool(TomlArray *arr, const char *key, unsigned int *ret)
{
	...
	for (size_t i = 0; i < arr->nitem; i++) {
		if (!strcmp(key, arr->item[i]->key)) {
			if (parsebool(arr->item[i], ret, errstr))
				return 1;
			return 0;
		}
	}
...
tomlgetdouble(TomlArray *arr, const char *key, double *ret)
{
	...
	for (size_t i = 0; i < arr->nitem; i++) {
		if (!strcmp(key, arr->item[i]->key)) {
			if (parsedouble(arr->item[i], ret, errstr))
				return 1;
			return 0;
		}
	}
...
tomlgetstring(TomlArray *arr, const char *key, char **ret)
{
	...
	for (size_t i = 0; i < arr->nitem; i++) {
		if (!strcmp(key, arr->item[i]->key)) {
			if (parsestring(arr->item[i], ret, errstr))
				return 1;
			return 0;
		}
	}
...
tomlgetuint(TomlArray *arr, const char *key, unsigned int *ret)
{
	...
	for (size_t i = 0; i < arr->nitem; i++) {
		if (!strcmp(key, arr->item[i]->key)) {
			if (parseuint(arr->item[i], ret, errstr))
				return 1;
			return 0;
		}
	}
...
main(void)
{
	...
	TomlArrayKey *key = tomlgetarraykey(main, "screen");
	TomlArrayKey *keymon;
	char *name;

	printf("Key:%s\n", key->key);
	for (size_t i = 0; i < key->narr; i++) {
		tomlgetstring(key->arr[i], "name", &name);
		printf("screen:%s\n", name);
		keymon = tomlgetarraykey(key->arr[i], "monitor");
		for (size_t j = 0; j < keymon->narr; j++) {
			tomlgetstring(keymon->arr[j], "id", &name);
			printf("\tmonitor:%s\n", name);
		}
	}
	tomldeletearray(main);
	return 0;
}