...
int
tomlgetstring(TomlArray *arr, const char *key, char * string)
{
	char *ret = NULL;
	char *errstr;
	...
	for (size_t i = 0; i < arr->nitem; i++) {
		if (!strcmp(key, arr->item[i]->key)) {
			parsestring(arr->item[i], &ret, errstr);
			return ret;
		}
	}
...
main(void)
{
	...
	for (size_t i = 0; i < key->narr; i++) {
		char *name = tomlgetstring(key->arr[i], "name");
		printf("screen:%s\n", name);
		keymon = tomlgetarraykey(key->arr[i], "monitor");
		for (size_t j = 0; j < keymon->narr; j++) {
			char *name = tomlgetstring(keymon->arr[j], "id");
			printf("\tmonitor:%s\n", name);
		}
	}
	deletearray(main);
