...
const char *matsrc = "Datasets/random-numbers-c.mat";
const char *matdst = "Result-data/c-knnsearch-result.mat";
...
int main(int argc, char *argv[]) {
	...
	sprintf(args, " %s %s", matsrc, matdst);
	if (blocks) {
		char temp[256];
		sprintf(temp, " %u", blocks);
		strcat(args, temp);
	}
