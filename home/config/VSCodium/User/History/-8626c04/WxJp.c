	...
	*ndev = 0;
	...
	if (DBUS_TYPE_ARRAY == dbus_message_iter_get_arg_type(&element)) {
		...
		while (dbus_message_iter_get_arg_type(&array_iter) == DBUS_TYPE_OBJECT_PATH) {
			...
			if (!(temp = realloc(ret, *ndev * sizeof(char*)))) {
				for (unsigned int i = 0; i < *ndev - 1; i++) {
					if (ret[i])
						free(ret[i]);
				}
				free(ret);
				dbus_message_unref(reply);
				logwrite("Function realloc() returned a NULL pointer.", NULL, LOG_ERROR, "dwmblocks-bluetooth");
				return NULL;
			}
			...
			if (!(temp[*ndev - 1] = malloc(256 * sizeof(char)))) {
				...
				free(ret);
				dbus_message_unref(reply);
				logwrite("Function malloc() returned a NULL pointer.", NULL, LOG_ERROR, "dwmblocks-bluetooth");
				return NULL;
			}
