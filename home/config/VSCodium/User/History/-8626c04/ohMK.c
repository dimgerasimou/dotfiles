...
static int
get_bluetooth_connected_devices(DBusConnection *conn, DBusError *err, char **dev, unsigned int *ndev)
{
	DBusMessage      *msg, *reply;
	...
	*ndev = 0;
	...
	if (!(dbus_message_iter_init(reply, &args) && DBUS_TYPE_VARIANT == dbus_message_iter_get_arg_type(&args))) {
		logwrite("Failed to parse the D-Bus reply message.", NULL, LOG_WARN, "dwmblocks-bluetooth");
		dbus_message_unref(reply);
		return -1;
	}
	...
	if (DBUS_TYPE_ARRAY == dbus_message_iter_get_arg_type(&element)) {
		...
		while (dbus_message_iter_get_arg_type(&array_iter) == DBUS_TYPE_OBJECT_PATH) {
			...
			if (!(temp = realloc(dev, *ndev * sizeof(char*)))) {
				dbus_message_unref(reply);
				logwrite("Function realloc() returned a NULL pointer.", NULL, LOG_FATAL, "dwmblocks-bluetooth");
			}
			
			if (!(temp[*ndev - 1] = malloc(256 * sizeof(char)))) {
				dbus_message_unref(reply);
				logwrite("Function malloc() returned a NULL pointer.", NULL, LOG_FATAL, "dwmblocks-bluetooth");
			}

			strncpy(temp[*ndev - 1], device_path, 256);
			dev = temp;
...
get_bluetooth_state(void)
{
	...
	dev = get_bluetooth_connected_devices(conn, &err, dev, &ndev);
