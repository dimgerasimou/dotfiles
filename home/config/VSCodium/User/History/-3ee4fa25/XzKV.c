...
uitoa(const unsigned int num)
{
	...
	if (sn_check < 0 || sn_check > (int) digits) {
		logwrite("uitoa() - snprintf() failed. Buffer overflow", NULL, LOG_ERROR, "dwmblocks-battery");
		return NULL;
	}
...
get_mode(void)
{
	...
	if (strstr(buffer, "integrated"))  ret = 1;
	else if (strstr(buffer, "hybrid")) ret = 2;
	else if (strstr(buffer, "nvidia")) ret = 3;
