	...
	while (!(str[ind] == ']' && !instr)) {
		...
		case '\0':
			...
			if (buf == EOF) {
				...
				logstring("ERROR - configuration fgetc() - EOF", "xrandr-setup");
				free(str);
				return NULL;
			}
			str[ind] = (char) buf;
...
setscreen(CfgScreen *s)
{
	...
	for (size_t i = 0; i < s->mc; i++) {
		if (s->m[i]->rid == 0) {
			setupemptyscreen(&s);
			logstring("WARN - Configuration error", "xrandr-setup");
			break;
		}
	}
...
promptoption(const char *menu, char *argv[])
{
	...
	if (pipe(writepipe) < 0 || pipe(readpipe) < 0) {
		logstring("Failed to initialize pipes", "xrandr-setup");
		return -ESTRPIPE;
	}
	
	switch (fork()) {
		case -1:
			logstring("fork() failed", "xrandr-setup");
			return -ECHILD;
...
getinputscreen(CfgScreens *cs, char *argv[])
{
	...
	for (size_t i = 0; i < cs->sc; i++) {
		...
		if ((size_t) ret > sizeof(buf) - 1 || ret < 0) {
			logstring("ERROR - snprintf() failed - buffer overflow (or encoding error)", "xrandr-setup");
			free(pstr);
			exit(errno);
		}
