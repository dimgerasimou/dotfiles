...
static double*
rankify(double *A, const uint32_t n, const double fpe)
{
...
static double
rank_correlation(void *A, void *B, const uint32_t n, const mat_type type, const double fpe)
{
	...
	double *rank_A = rankify(At, n, fpe);
	double *rank_B = rankify(Bt, n, fpe);
...
static void
comparedistances(IOHeader headerA, IOHeader headerB, const uint16_t sort, const double fpe)
{
	Matrix_t *A, *B;
	uint64_t  mismatched;
	double    inaccuracy;
	...
	case DOUBLE:{
		...
		for (uint32_t i = 0; i < A->rows; i++) {
			...
			rank_correlation_sum += rank_correlation(A, B, A->cols, DOUBLE, fpe);
			...
			s_rank_correlation_sum += rank_correlation(A, B, A->cols, DOUBLE, fpe);
	...
	case SINGLE:
		...
		for (uint32_t i = 0; i < A->rows; i++) {
			...
			rank_correlation_sum += rank_correlation(A, B, A->cols, SINGLE, fpe);
			...
			s_rank_correlation_sum += rank_correlation(A, B, A->cols, SINGLE, fpe);
	...
	printf("\tFloating point error set:       %.2e\n", fpe);
	printf("\tAccuracy:                       %.4e\n", 1 - inaccuracy / A->rows);
	printf("\tMax error:                      %.4e\n", max_error);
	printf("\tMean Absolute Error:            %.4e\n", difference_sum / (A->rows * A->cols));
	printf("\tMean Square Error:              %.4e\n", difference_sq_sum * difference_sum / (A->rows * A->cols));
	printf("\tMean Relative Error:            %.4e\n", relative_error_sum / (A->rows * A->cols));
	printf("\tMean Rank correlation:          %.4e\n", rank_correlation_sum / (A->rows * A->cols));
	printf("\tMean Sorted Rank correlation:   %.4e\n", s_rank_correlation_sum / (A->rows * A->cols));
...
static void
compareneighbors(IOHeader headerA, IOHeader headerB, const uint16_t sort)
{
	Matrix_t *A, *B;
	...
	for (uint32_t i = 0; i < A->rows; i++) {
		...
		rank_correlation_sum += rank_correlation(A_temp, B_temp, A->cols, INT32, 0);
	...
	printf("\tAccuracy:                       %.4e\n", 1 - inaccuracy / A->rows);
	printf("\tMean top K overlap:             %.4e\n", top_k_overlap_sum / A->rows);
	printf("\tMean unsorted rank correlation: %.4e\n", rank_correlation_sum / A->rows);
	printf("\tMean Jaccard index:             %.4e\n", jaccard_index_sum / A->rows);
...
static void
parseargs(int argc, char *argv[], IOHeader *headerA, IOHeader *headerB, double *fpe, uint16_t *sort)
{
...
main(int argc, char *argv[])
{
	...
	parseargs(argc, argv, &headerA, &headerB, &fpe, &sort);

	compareneighbors(headerA, headerB, sort);
	comparedistances(headerA, headerB, fpe, sort);
