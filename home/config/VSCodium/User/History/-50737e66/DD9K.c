...
{
	int overlap = 0;
	int32_t *top_k_A = mmalloc(k * sizeof(int32_t));
	int32_t *top_k_B = mmalloc(k * sizeof(int32_t));

	k = n < k ? n : k;

	memcpy(top_k_A, A, n * sizeof(int32_t));
	qsort(top_k_A, n, sizeof(int32_t), compareint32);

	memcpy(top_k_B, B, n * sizeof(int));
	qsort(top_k_B, n, sizeof(int), compareint32);

	for (uint32_t i = 0; i < k; i++) {
		for (uint32_t j = 0; j < k; j++) {
			if (top_k_A[i] == top_k_B[j]) {
				overlap++;
				break;
			}
		}
