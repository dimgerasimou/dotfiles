...
rank_correlation(int32_t *A, int32_t *B, uint32_t n)
{
	...
	int32_t *A_sorted_indices = mmalloc(n * sizeof(int32_t));
	int32_t *B_sorted_indices = mmalloc(n * sizeof(int32_t));

	// Fill in indices for sorting
	for (uint32_t i = 0; i < n; i++) {
		A_sorted_indices[i] = i;
		B_sorted_indices[i] = i;
	}

	// Sort based on values in A and B
	qsort(A_sorted_indices, n, sizeof(int32_t), compareint32);
	qsort(B_sorted_indices, n, sizeof(int32_t), compareint32);

	// Rank assignment (handling ties)
	for (uint32_t i = 0; i < n; i++) {
		rank_A[A_sorted_indices[i]] = i + 1;
		rank_B[B_sorted_indices[i]] = i + 1;
	}
	...
	free(rank_A);
	free(rank_B);
	free(A_sorted_indices);
	free(B_sorted_indices);
...
compareneighbors(IOHeader headerA, IOHeader headerB)
{
	...
	printf("\tAccuracy:                 %.8lf\n", 1 - inaccuracy / A->rows);
	printf("\tAverage top K overlap:    %.8lf\n", top_k_overlap_sum / A->rows);
	printf("\tAverage rank correlation: %.8lf\n", rank_correlation_sum / A->rows);
	printf("\tAverage Jaccard index:    %.8lf\n", jaccard_index_sum / A->rows);
