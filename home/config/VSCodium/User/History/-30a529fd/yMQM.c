...
hdf5getmat(const char *filepath, const char *varname)
{
	...
	file_id = H5F_open(filepath, H5F_ACC_RDONLY, H5P_DEFAULT);
	if (file_id < 0)
		die("Invalid filepath");

	var = H5D_open(file_id, varname, H5P_DEFAULT);
	if (var < 0) {
		H5F_close(file_id);
		die("Invalid filepath");
	}

	dataspace_id = H5D_get_space(var);
	H5S_get_simple_extent_dims(dataspace_id, dims, NULL);
