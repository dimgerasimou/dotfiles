	...
	case H5T_INTEGER: {
		if (H5Tget_size(datatype_id) == 4) {
			...
			for (hsize_t i = 0; i < dims[0] * dims[1]; i++) {
                ...
                free(temp_data);
            } else if (H5Tget_size(datatype_id) == 2) {  // 16-bit integer (short)
                short *temp_data = (short *)malloc(dims[0] * dims[1] * sizeof(short));
                status = H5D_read(dataset_id, H5T_NATIVE_SHORT, H5S_ALL, H5S_ALL, H5P_DEFAULT, temp_data);
                if (status < 0) {
                    fprintf(stderr, "Error reading short data.\n");
                    free(temp_data);
                    return -1;
                }
                // Convert to double
                for (hsize_t i = 0; i < dims[0] * dims[1]; i++) {
                    (*data)[i] = (double)temp_data[i];
                }
                free(temp_data);
            }
            break;
        ...
        case H5T_FLOAT: {
            if (H5Tget_size(datatype_id) == 4) {  // 32-bit float
                ...
                status = H5D_read(dataset_id, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, temp_data);
                if (status < 0) {
                    fprintf(stderr, "Error reading float data.\n");
                    free(temp_data);
                    return -1;
                }
                // Convert to double
                ...
                status = H5D_read(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, *data);
                if (status < 0) {
                    fprintf(stderr, "Error reading double data.\n");
                    return -1;
                }
            }
            break;
        ...
        case H5T_STRING: {
            // If the dataset is a string type, we will not handle it here as it's not a numerical type
            fprintf(stderr, "Error: The dataset is of string type, which cannot be converted to double.\n");
            return -1;
        }
