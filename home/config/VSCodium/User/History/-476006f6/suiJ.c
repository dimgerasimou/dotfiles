...
static void neighbors_swap(Neighbor *a, Neighbor *b);
 ...
 */
static void
getdistance(const float *C, const float *Q, const Matrix_t *D, float *D_M, const float *Cnorm, const float *Qnorm, const uint32_t cols)
{
		...
		for (uint32_t i = 0; i < Q->rows; i += blocksize) {
			...
			for (uint32_t j = 0; j < C->rows; j += blocksize) {
				...
				for (uint32_t ii = 0; ii < D->rows; ii++) {
					...
					if (*K < j && neighbors_row[*K - 1].dst == neighbors_row[*K].dst && neighbors_row[*K - 1].idx > neighbors_row[*K].idx)
...
static void
neighbors_swap(Neighbor *a, Neighbor *b)
{
	Neighbor temp = *a;
	*a = *b;
	*b = temp;
...
qselect_partition(Neighbor *neighbors, const int left, const int right)
{
	...
	for (int j = left; j < right; j++) {
		if (neighbors[j].dst < pivot) {
			neighbors_swap(&neighbors[i], &neighbors[j]);
			i++;
		}
	}

	neighbors_swap(&neighbors[i], &neighbors[right]);
	return i;
}
...
static void
qselect_qsort(Neighbor *neighbors, const int left, const int right)
{
	if (left < right) {
