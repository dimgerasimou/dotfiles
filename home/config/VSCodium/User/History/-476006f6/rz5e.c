	...
	*K = C->rows < *K ? C->rows : *K;
		...
		for (uint32_t i = 0; i < Q->rows; i += blocksize) {
			...
			for (uint32_t j = 0; j < C->rows; j += blocksize) {
				...
				for (uint32_t ii = 0; ii < D->rows; ii++) {
					...
					if (neighbors[((ii + i) * *K) + *K].dst == neighbors_row[*K + 1].dst &&
					    neighbors[((ii + i) * *K) + *K].idx > neighbors_row[*K + 1].idx)
						qselect_swap(&neighbors[((ii + i) * *K) + *K], &neighbors_row[*K + 1]);
				}
			}
		}
	}
	...
	for (uint32_t i = 0; i < Q->rows; i++) {
		...
		for (uint32_t j = 0; j < *K; j++) {
			...
			if (j > 0 && dst_temp[(i * *K) + j] == dst_temp[(i * *K) + j - 1] && idx_temp[(i * *K) + j] < idx_temp[(i * *K) + j - 1]) {
				int32_t tempi = idx_temp[(i * *K) + j];

				idx_temp[(i * *K) + j] = idx_temp[(i * *K) + j - 1];
				idx_temp[(i * *K) + j - 1] = tempi;
			}
		}
	}
