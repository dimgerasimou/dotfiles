	...
	*K = C->rows < *K ? C->rows : *K;
	...
	for (uint32_t i = 0; i < Q->rows; i++) {
		...
		for (uint32_t j = 0; j < *K; j++) {
			...
			if (j > 0 && neighbors[(i * *K) + j].dst == neighbors[(i * *K) + j - 1].dst && idx_temp[(i * *K) + j - 1] > neighbors[(i * *K) + j].idx) {
				idx_temp[(i * *K) + j] = idx_temp[(i * *K) + j - 1];
				idx_temp[(i * *K) + j - 1] = neighbors[(i * *K) + j].idx;
			}
		}
	}
