#include <math.h>
#include <stdint.h>
#include <stdio.h>
#include <string.h>
#include <sys/time.h>
#include <sys/types.h>
#include <time.h>
#include <unistd.h>
#include <openblas/cblas.h>
#include <omp.h>

#include "knnsearch.h"

/* global variables */
uint16_t g_blocksize = 0;

/*
 * dynamically adjusts the blocksize to a power of 2, based on the cache size:
 * total memory = (2 * g_blocksize * cols + 2 * g_blocksize^2 + 2 * g_blocksize) * sizeof(double).
 *
 * For over 2^27 bytes in the D matrix returns a g_blocksize, where total memory exceedes
 * the L2 cache, even though of the increased probability of cache misses, the penalty on average is less
 * than the cost of reducing the iterations by half. For elements less than 2^27, g_blocksize
 * is set that the total memory usage of each thread is less than the L2 cache.
*/
void
dynamic_blocksize(size_t element_size, uint32_t cols, uint64_t nelem)
{
	long cache_size;
	double max_blocksize;

	// check if 
	cache_size = sysconf(_SC_LEVEL2_CACHE_SIZE);
	if (cache_size == -1) cache_size = 512 * 1024;  // default to 512 KiB if unknown

	max_blocksize = (- (double) (cols + 1) + sqrt((double) (cols + 1) * (cols + 1) + (double) (2 * cache_size) / element_size)) / 2.0;

	// round to nearest power of 2 for block alignment
	uint16_t ret = 1;
	while (ret < (int) max_blocksize) ret *= 2;

	if (nelem < 134217728 / element_size)
		ret /= 2;

	g_blocksize =  ret > 32 ? ret : 32;  // return 32 as a safe fallback
}

/*
 * calculates the norms of the given matrix
 */
void
getnormsdouble(Matrix *mat, double *norms)
{
	#pragma omp parallel for
	for (uint32_t i = 0; i < mat->rows; i++)
		norms[i] = cblas_ddot(mat->cols, &((double*)mat->data)[i * mat->cols], 1, &((double*)mat->data)[i * mat->cols], 1);
}

void
getnormssingle(Matrix *mat, float *norms)
{
	#pragma omp parallel for
	for (uint32_t i = 0; i < mat->rows; i++)
		norms[i] = cblas_sdot(mat->cols, &((float*) mat->data)[i * mat->cols], 1, &((float*) mat->data)[i * mat->cols], 1);
}

/*
 * using cblas gets the euclidian distance of the vectors of the first matrix (rows)
 * compared with the vectors of the second (rows) and returns in a matrix.
 */
void
getdistancedouble(const double *C, const double *Q, const Matrix *D, double *D_M, double *Cnorm, double *Qnorm, const uint32_t cols)
{
	cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasTrans, D->cols, D->rows, cols, -2.0,
	            C, cols, Q, cols, 0.0, D_M, D->rows);

	for (uint32_t i = 0; i < D->rows; i++) {
		for (uint32_t j = 0; j < D->cols; j++) {
			((double*) D->data)[i * D->cols + j] = Cnorm[j] + D_M[j * D->rows + i] + Qnorm[i];
		}
	}
}

void
getdistancesingle(const float *C, const float *Q, const Matrix *D, float *D_M, float *Cnorm, float *Qnorm, const uint32_t cols)
{
	cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans, D->cols, D->rows, cols, -2.0,
	            C, cols, Q, cols, 0.0, D_M, D->rows);

	for (uint32_t i = 0; i < D->rows; i++) {
		for (uint32_t j = 0; j < D->cols; j++) {
			((float*) D->data)[i * D->cols + j] = Cnorm[j] + D_M[j * D->rows + i] + Qnorm[i];
		}
	}
}

/*
 * returns the k - nearest neighbors of the vector querries (rows of Q)
 * compared to the vectors of the corpus set (rows of C)
 */
Neighbors*
knnsearchdouble(Matrix *C, Matrix *Q, uint32_t *K)
{
	uint16_t nthreads;
	Neighbor **neighbors_b;
	Neighbors *neighbors;
	double   *Cnorm, *Qnorm, **D_M_b;
	Matrix   **D_b;

	if (Q->cols != C->cols) {
		printf("Dimensions are not the same\n");
		matrixfree(&C);
		matrixfree(&Q);
		exit(0);
	}

	*K = C->rows < *K ? C->rows : *K;
	nthreads    = omp_get_max_threads();
	neighbors   = neighborsinit(Q->rows, *K, DOUBLE);
	Cnorm       = mmalloc(C->rows * sizeof(double));
	Qnorm       = mmalloc(Q->rows * sizeof(double));
	D_b         = mmalloc(nthreads * sizeof(Matrix*));
	D_M_b       = mmalloc(nthreads * sizeof(double*));
	neighbors_b = mmalloc(nthreads * sizeof(Neighbor *));

	getnormsdouble(C, Cnorm);
	getnormsdouble(Q, Qnorm);

	#pragma omp parallel
	{
		uint16_t tID = omp_get_thread_num();

		D_b[tID] =  matrixinit(g_blocksize, g_blocksize, DOUBLE);
		neighbors_b[tID] = mmalloc((g_blocksize + *K) * sizeof(Neighbor));
		D_M_b[tID] = mmalloc(g_blocksize * g_blocksize * sizeof(double));

		#pragma omp for
		for (uint32_t i = 0; i < Q->rows; i += g_blocksize) {
			Matrix *D = D_b[tID];
			Neighbor *neighbors_row = neighbors_b[tID];
			double *D_M = D_M_b[tID];
			
			D->rows = (i + g_blocksize > Q->rows) ? (Q->rows - i) : g_blocksize;

			for (uint32_t j = 0; j < C->rows; j += g_blocksize) {
				D->cols = (j + g_blocksize > C->rows) ? (C->rows - j) : g_blocksize;
				getdistancedouble(((double*)C->data) + j * C->cols, ((double*)Q->data) + i * Q->cols, D, D_M, &Cnorm[j], &Qnorm[i], C->cols);

				uint32_t coloffset = j < *K ? j : *K;
				for (uint32_t ii = 0; ii < D->rows; ii++) {
					for (uint32_t jj = 0; jj < D->cols; jj++) {
						neighbors_row[coloffset + jj].idx = j + jj;
						neighbors_row[coloffset + jj].dst = ((double*)D->data)[jj + ii * D->cols];
					}

					for (uint32_t jj = 0; jj < coloffset; jj++) {
						neighbors_row[jj].dst = ((double*) neighbors->dst)[((ii + i) * *K) + jj];
						neighbors_row[jj].idx = ((int32_t*) neighbors->idx)[((ii + i) * *K) + jj];
					}

					//qselect(neighbors_row, 0, coloffset + D->cols - 1, *K);

					for (uint32_t idx = 0; idx < *K; idx++)  {
						((int32_t*) neighbors->dst)[((ii + i) * *K) + idx] = neighbors_row[idx].dst;
						((double*) neighbors->idx)[((ii + i) * *K) + idx] = neighbors_row[idx].idx;
					}
				}
			}
		}
	}

	for (int i = 0; i < nthreads; i++) {
		free(neighbors_b[i]);
		matrixfree(&D_b[i]);
		free(D_M_b[i]);
	}

	free(neighbors_b);
	free(D_b);
	free(D_M_b);
	free(Cnorm);
	free(Qnorm);

	#pragma omp parallel for
	for (uint32_t i = 0; i < Q->rows; i++) {
		qSort(neighbors, i * *K, (i * *K) + *K - 1);
		for (uint32_t j = 0; j < *K; j++) {
			((double*) neighbors->dst)[(i * *K) + j] = sqrt(((double*) neighbors->dst)[(i * *K) + j]);
		}
	}

	return neighbors;
}

Neighbors*
knnsearchsingle(Matrix *C, Matrix *Q, uint32_t *K)
{
	uint16_t nthreads;
	Neighbors **neighbors_b;
	Neighbors *neighbors;
	float   *Cnorm, *Qnorm, **D_M_b;
	Matrix   **D_b;

	if (Q->cols != C->cols) {
		printf("Dimensions are not the same\n");
		matrixfree(&C);
		matrixfree(&Q);
		exit(0);
	}

	*K = C->rows < *K ? C->rows : *K;
	nthreads    = omp_get_max_threads();
	neighbors   = neighborsinit(Q->rows, *K, SINGLE);
	Cnorm       = mmalloc(C->rows * sizeof(float));
	Qnorm       = mmalloc(Q->rows * sizeof(float));
	D_b         = mmalloc(nthreads * sizeof(Matrix*));
	D_M_b       = mmalloc(nthreads * sizeof(float*));
	neighbors_b = mmalloc(nthreads * sizeof(Neighbors *));

	getnormssingle(C, Cnorm);
	getnormssingle(Q, Qnorm);

	#pragma omp parallel
	{
		uint16_t tID = omp_get_thread_num();

		D_b[tID] =  matrixinit(g_blocksize, g_blocksize, SINGLE);
		neighbors_b[tID] = neighborsinit(1, (g_blocksize + *K), SINGLE);
		D_M_b[tID] = mmalloc(g_blocksize * g_blocksize * sizeof(float));

		#pragma omp for
		for (uint32_t i = 0; i < Q->rows; i += g_blocksize) {
			Matrix *D = D_b[tID];
			Neighbors *neighbors_row = neighbors_b[tID];
			float *D_M = D_M_b[tID];
			
			D->rows = (i + g_blocksize > Q->rows) ? (Q->rows - i) : g_blocksize;

			for (uint32_t j = 0; j < C->rows; j += g_blocksize) {
				D->cols = (j + g_blocksize > C->rows) ? (C->rows - j) : g_blocksize;
				getdistancesingle(((float*)C->data) + j * C->cols, ((float*)Q->data) + i * Q->cols, D, D_M, &Cnorm[j], &Qnorm[i], C->cols);

				uint32_t coloffset = j < *K ? j : *K;
				for (uint32_t ii = 0; ii < D->rows; ii++) {
					for (uint32_t jj = 0; jj < D->cols; jj++) {
						((int32_t*) neighbors->idx)[coloffset + jj] = j + jj;
						((float*) neighbors->dst)[coloffset + jj] = ((float*)D->data)[jj + ii * D->cols];
					}

					for (uint32_t jj = 0; jj < coloffset; jj++) {
						((float*) neighbors->dst)[jj] = ((float*) neighbors->dst)[((ii + i) * *K) + jj];
						((int32_t*) neighbors->idx)[jj]  = ((int32_t*) neighbors->idx)[((ii + i) * *K) + jj];
					}

					qselect(neighbors_row, 0, coloffset + D->cols - 1, *K);

					for (uint32_t idx = 0; idx < *K; idx++)  {
						((float*) neighbors->dst)[((ii + i) * *K) + idx] = ((float*) neighbors->dst)[idx];
						((int32_t*) neighbors->idx)[((ii + i) * *K) + idx] = ((int32_t*) neighbors->idx)[idx];
					}
				}
			}
		}
	}

	for (int i = 0; i < nthreads; i++) {
		neighborsfree(&neighbors_b[i]);
		matrixfree(&D_b[i]);
		free(D_M_b[i]);
	}

	free(neighbors_b);
	free(D_b);
	free(D_M_b);
	free(Cnorm);
	free(Qnorm);

	#pragma omp parallel for
	for (uint32_t i = 0; i < Q->rows; i++) {
		qSort(neighbors, i * *K, (i * *K) + *K - 1);
		for (uint32_t j = 0; j < *K; j++) {
			((float*) neighbors->dst)[(i * *K) + j] = sqrt(((float*) neighbors->dst)[(i * *K) + j]);
		}
	}

	return neighbors;
}

Neighbors*
knnsearch(Matrix *C, Matrix *Q, uint32_t *K, enum mat_type *type)
{
	if ((C->type == INT16 || C->type == SINGLE) && (Q->type == INT16 || Q->type == SINGLE)) {
		matrixtosingle(C);
		matrixtosingle(Q);
		*type = SINGLE;

		if (!g_blocksize)
			dynamic_blocksize(sizeof(float), C->cols, C->rows * Q->rows);

		return knnsearchsingle(C, Q, K);
	}

	matrixtodouble(C);
	matrixtodouble(Q);
	*type = DOUBLE;

	if (!g_blocksize)
		dynamic_blocksize(sizeof(double), C->cols, C->rows * Q->rows);

	return knnsearchdouble(C, Q, K);
}

void
parseargs(int argc, char *argv[], IOHeader *input, IOHeader *output, uint32_t *K)
{
	for (int i = 1; i < argc; i++) {
		if (!strcmp(argv[i], "-i")) {
			if (argc < i + 2)
				die("Invalid number of arguments");

			if (!strcmp(argv[++i], "mat"))
				input->type = MAT;
			else if
				(!strcmp(argv[i], "hdf5")) input->type = HDF5;
			else
				die("Invalid arguments");

			input->path = strdup(argv[++i]);
		} else if (!strcmp(argv[i], "-o")) {
			if (argc < i + 2)
				die("Invalid number of arguments");

			if (!strcmp(argv[++i], "mat"))
				output->type = MAT;
			else if
				(!strcmp(argv[i], "hdf5")) output->type = HDF5;
			else
				die("Invalid arguments");

			output->path = strdup(argv[++i]);
		} else if (!strcmp(argv[i], "-b")) {
			if (argc < i + 1)
				die("Invalid number of arguments");
			if (isuint(argv[++i]))
				g_blocksize = atoi(argv[i]);
		} else if (!strcmp(argv[i], "-k")) {
			if (argc < i + 1)
				die("Invalid number of arguments");
			if (isuint(argv[++i]))
				*K = atoi(argv[i]);
		} else {
			die("Invalid argument");
		}
	}

	if (!input->type || !output->type)
		die("Did not specify input - output files");
}

int
main(int argc, char *argv[])
{
	struct timeval t0, t1;
	Neighbors *neighbors;
	uint32_t K = 0;
	Matrix   *C = NULL;
	Matrix   *Q = NULL;
	IOHeader input = {0, NULL}, output = {0, NULL};
	enum mat_type result_type;

	parseargs(argc, argv, &input, &output, &K);

	if (input.type == HDF5) {
		if (!K)
			die("Enter a K value");
		C = matrixget(input, "train");
		Q = matrixget(input, "test");
	} else if(input.type == MAT) {
		C = matrixget(input, "C");
		Q = matrixget(input, "Q");
		if (!K)
			K = uintget(input, "K");
	} else {
		die("Did not initialize properly");
	}

	gettimeofday(&t0, NULL);

	// benchmarked function here
	neighbors = knnsearch(C, Q,&K, &result_type);

	gettimeofday(&t1, NULL);
	
	printf("%.8g\n", t1.tv_sec - t0.tv_sec + 1E-6 * (t1.tv_usec - t0.tv_usec));

	neighborssave(neighbors, output);
	
	neighborsfree(&neighbors);
	matrixfree(&C);
	matrixfree(&Q);
	return 0;
}
