 ...
 * for a total number of dst size, exceeding around 2^24, the total number of loops penalty, overshadow the 
 * cache miss penalty. It has been tested that for less than 2^24 elements, L2 > total memory, is faster,
 * where 
*/
void
dynamic_blocksize(size_t element_size, uint32_t cols, uint64_t nelems)
{
	long cache_size;
