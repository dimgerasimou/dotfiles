...
execbutton(AudioInfo **a)
{
	...
	case 2:
		forkexecvp((char**) eqalizer_args, "dwmblocks-volume");
		break;

	case 3:
		path = get_path((char**) volume_control_path, 1);
		forkexecv(path, (char**) volume_mute_args, "dwmblocks-volume");
		free(path);
		break;

	case 4:
		path = get_path((char**) volume_control_path, 1);
		forkexecv(path, (char**) volume_increase_args, "dwmblocks-volume");
		free(path);
		break;

	case 5:
		path = get_path((char**) volume_control_path, 1);
		forkexecv(path, (char**) volume_decrase_args, "dwmblocks-volume");
		free(path);
		break;
