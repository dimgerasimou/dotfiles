...
#define BUCKET_COUNT 60385 // Adjust based on your needs
...
lshinit(const double W, const uint32_t L, const uint32_t K, const uint32_t dims, const uint32_t C)
{
	LshParameters *ret;
...
candidatescollect(Matrix_t *Q, LshParameters *lsh, HashTable **tables, uint32_t *hashes, uint32_t **candidates, uint32_t *candidate_sizes)
{
    ...
    uint8_t **visited;  // Allocate visited for each table
    uint8_t *added_candidates;  // Track added candidates

    // Allocate visited for each table L
    visited = mmalloc(lsh->L * sizeof(uint8_t*));
    for (uint32_t t = 0; t < lsh->L; t++) {
        visited[t] = mmalloc(BUCKET_COUNT * sizeof(uint8_t));  // Each table has its own visited array
    }

    // Allocate an array to track added candidates (can be used to avoid duplicates)
    added_candidates = mmalloc(lsh->C * sizeof(uint8_t));  // Assuming C->rows is the max number of candidates

    for (uint32_t q = 0; q < Q->rows; q++) {
        count = 0;
        memset(candidates[q], 0, lsh->C * sizeof(uint32_t));  // Reset the candidates list for query q

        // Initialize visited arrays for each hash table for this query
        for (uint32_t t = 0; t < lsh->L; t++) {
            memset(visited[t], 0, BUCKET_COUNT * sizeof(uint8_t));
        }

        // Initialize added_candidates to track which candidates are added for this query
        memset(added_candidates, 0, lsh->C * sizeof(uint8_t));

        // Debugging: print which query we're processing
        printf("Processing query %u\n", q);

        // For each hash table (L tables)
        for (uint32_t t = 0; t < lsh->L; t++) {
            bucket_idx = 0;

            // Compute the bucket index for this query and table
            for (uint32_t h = 0; h < lsh->K; h++) {
                bucket_idx ^= hashes[(q * lsh->L + t) * lsh->K + h];
            }
            bucket_idx %= BUCKET_COUNT;

            // Debugging: print the computed bucket index
            printf("  Table %u: Bucket index for query %u: %u\n", t, q, bucket_idx);

            // Avoid duplicate candidates from the same bucket (only for this table)
            if (!visited[t][bucket_idx]) {
                visited[t][bucket_idx] = 1;
                // Debugging: print the size of the bucket before inserting candidates
                printf("    Adding candidates from bucket %u of table %u:\n", bucket_idx, t);

                // Add candidates from this bucket to the list for query q
                for (uint32_t j = 0; j < tables[t]->bucket_sizes[bucket_idx]; j++) {
                    uint32_t candidate = tables[t]->bucket_data[bucket_idx][j];

                    // Check if the candidate is already added
                    if (!added_candidates[candidate]) {
                        // Print the candidate being added to track duplicates
                        printf("      Adding candidate %u\n", candidate);

                        candidates[q][count++] = candidate;
                        added_candidates[candidate] = 1;  // Mark candidate as added
                    }
                }
            }
        }

        // Store the number of candidates for this query
        candidate_sizes[q] = count;

        // Debugging: print how many candidates were collected for this query
        printf("  Query %u has %u candidates\n", q, count);
    }

    // Free the visited arrays
    for (uint32_t t = 0; t < lsh->L; t++) {
        free(visited[t]);
    }
    free(visited);

    // Free added_candidates array
    free(added_candidates);
}
...
knnsearch(Matrix_t *C, Matrix_t *Q, uint32_t *K, uint32_t blocksize, uint16_t quick_sort)
{
	...
	lsh = lshinit(w, 2, 2, C->cols, C->rows);
	...
	printf("Buckets:\n");
	for (uint i = 0; i < lsh->L; i++) {
		printf("Table %u:\n", i);

		for (uint j = 0; j < tables[i]->nbuckets; j++) {
			printf("\tBucket %u:", j);

			for (uint k = 0; k < tables[i]->bucket_sizes[j]; k++) {
				printf(" %u", tables[i]->bucket_data[j][k]);
			}
		}
		printf("\n");
	}
