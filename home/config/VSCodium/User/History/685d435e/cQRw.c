...
#define BUCKET_COUNT 65536 // Adjust based on your needs
#define INITIAL_BUCKET_SIZE 16 // Initial bucket capacity
#define NUM_HASH_FUNCTIONS 10 // Number of hash functions
	...
	for (uint32_t q = 0; q < Q->rows; q++) {
		...
		for (uint32_t t = 0; t < lsh->L; t++) {
			...
			for (uint32_t h = 0; h < lsh->K; h++) {
				bucket_idx += bucket_idx * 31 + hashes[(q * lsh->L + t) * lsh->K + h];
			}
			...
			if (!visited[bucket_idx]) {
				...
				printf("bucket_size:%u\n", tables[t]->bucket_sizes[bucket_idx]);
...
knnsearch(Matrix_t *C, Matrix_t *Q, uint32_t *K, uint32_t blocksize, uint16_t quick_sort)
{
	...
	for (uint32_t i = 0; i < C->rows; i++) {
		for (uint32_t t = 0; t < lsh->L; t++) {
			...
			for (uint32_t h = 0; h < lsh->K; h++) {
				bucket_idx = bucket_idx * 31 + hashes[(i * lsh->L + t) * lsh->K + h];
			}
			bucket_idx %= BUCKET_COUNT;
	...
	for (uint32_t q = 0; q < Q->rows; q++) {
		...
		for (uint32_t k = 0; k < *K; k++) {
			printf("idx%d, dst%lf - ", neighbors[k].idx, neighbors[k].dst);
